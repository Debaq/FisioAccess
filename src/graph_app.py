...
class GraphApp:
    def __init__(self):
        ...
        """
        self.data = deque(maxlen=MAX_POINTS * 2)  # Permitir hasta el doble de MAX_POINTS
        self.view_start = 0
        ...
        self.current_max_points = MAX_POINTS  # Nueva variable para controlar el límite actual
        ...
        if self.record and self.played:
            ...
            if len(self.data) >= self.current_max_points:
                if self.current_max_points < MAX_POINTS * 2:
                    ...
                    self.current_max_points = min(self.current_max_points * 2, MAX_POINTS * 2)
    ...
    def draw_grid(self, screen):
        ...
        for x in range(self.margin_left, WIDTH - self.margin_right, int(self.graph_width / (VISIBLE_POINTS / self.grid_ms_per_div))):
            pygame.draw.line(screen, grid_color, (x, self.margin_top), (x, HEIGHT - self.margin_bottom), 1)
        ...
        if data_range > 0:
            ...
            y_step = max(1, int(y_div_height * self.scale_factor))  # Asegurarse de que no sea 0
            for y in range(self.margin_top, HEIGHT - self.margin_bottom, y_step):
                pygame.draw.line(screen, grid_color, (self.margin_left, y), (WIDTH - self.margin_right, y), 1)
    ...
    def draw_graph(self, screen, **kwargs):
        ...
        if visible_data:
            ...
            for i in range(1, len(visible_data)):
                ...
                y1 = HEIGHT - self.margin_bottom - int((visible_data[i-1] - min_value) * self.scale_factor)
                x2 = self.margin_left + int(i * self.graph_width / VISIBLE_POINTS)
                y2 = HEIGHT - self.margin_bottom - int((visible_data[i] - min_value) * self.scale_factor)
    ...
    def draw_selection(self, screen):
        pygame.draw.line(screen, self.selection_color, (self.selection_start, self.margin_top), (self.selection_start, HEIGHT - self.margin_bottom), 2)
        pygame.draw.line(screen, RED, (self.selection_start, self.margin_top), (self.selection_start, HEIGHT - self.margin_bottom), 2)
        pygame.draw.line(screen, self.selection_color, (self.selection_end, self.margin_top), (self.selection_end, HEIGHT - self.margin_bottom), 2)
        pygame.draw.line(screen, RED, (self.selection_end, self.margin_top), (self.selection_end, HEIGHT - self.margin_bottom), 2)

    def draw_selection_view(self, screen):
        for i in range(1, len(self.selected_data)):
            x1 = self.margin_left + int((i-1) * self.graph_width / len(self.selected_data))
            y1 = self.margin_top + self.selected_data[i-1]
            x2 = self.margin_left + int(i * self.graph_width / len(self.selected_data))
    ...
    def set_selection(self, start, end):
        self.selection_start = max(self.margin_left, min(start, WIDTH - self.margin_right))
        self.selection_end = max(self.margin_left, min(end, WIDTH - self.margin_right))

    def get_selected_data(self):
        if self.selection_start is None or self.selection_end is None:
            return []
        start_index = int((self.selection_start - self.margin_left) * VISIBLE_POINTS / self.graph_width)
        end_index = int((self.selection_end - self.margin_left) * VISIBLE_POINTS / self.graph_width)
    ...
    def find_nearest_point(self, x):
        x = max(self.margin_left, min(x, WIDTH - self.margin_right))
        index = int((x - self.margin_left) * len(self.selected_data) / self.graph_width)
