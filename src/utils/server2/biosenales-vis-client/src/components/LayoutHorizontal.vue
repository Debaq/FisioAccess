<template>
  <div class="layout-horizontal">
    <div id="visualization-container" class="visualization-container">
      ...
      <!-- Separador (resizable) -->
      <div class="resizer" @mousedown="startResize" :style="{ left: `${splitPosition}%` }"></div>
...
export default {
  ...
  setup(props) {
    ...
    // Posición del separador (por defecto 60%)
    const splitPosition = ref(60)
    ...
    const onResize = (event) => {
      ...
      const relativePosition = ((event.clientX - containerRect.left) / containerRect.width) * 100
    ...
    const updatePanelSizes = () => {
      ...
      if (mainPanel && secondaryPanel) {
        mainPanel.style.width = `${splitPosition.value}%`
        secondaryPanel.style.width = `${100 - splitPosition.value}%`
        secondaryPanel.style.left = `${splitPosition.value}%`
      }
    }
...
.main-panel {
  ...
  top: 0;
  left: 0;
  width: 60%; /* Por defecto, se actualiza con JS */
  height: 100%;
  overflow: auto;
  padding: 10px;
  z-index: 1;
...
.secondary-panel {
  position: absolute;
  top: 0;
  left: 60%; /* Por defecto, se actualiza con JS */
  width: 40%; /* Por defecto, se actualiza con JS */
  height: 100%;
  overflow: auto;
  padding: 10px;
